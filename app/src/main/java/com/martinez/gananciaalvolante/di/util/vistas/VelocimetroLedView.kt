...
class VelocimetroLedView @JvmOverloads constructor(
    ...
    // --- NUEVAS PROPIEDADES PARA EL LED ---
    private var isRecording = false
    private val paintLed = Paint(Paint.ANTI_ALIAS_FLAG).apply { style = Paint.Style.FILL }
    private val colorLedOff = Color.RED
    private val colorLedOn = Color.GREEN
    private var ledAnimator: ValueAnimator? = null
    private var textoDeEstado = "TOCA PARA INICIAR"

    // --- NUEVAS PROPIEDADES PARA LA DESCUBRIBILIDAD ---
    private val paintDiscoverabilityText = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        textAlign = Paint.Align.CENTER
        typeface = Typeface.create(Typeface.SANS_SERIF, Typeface.NORMAL)
    }
    private var discoverabilityAnimator: ValueAnimator? = null
    private var discoverabilityScale = 1f
    private var discoverabilityAlpha = 1f
    private var currentUnit = DistanceUnit.KM // Valor por defecto
    private var textoUnidadVelocidad = "km/h"
    ...
    init {
        attrs?.let {
            ...
            paintAguja.color =
                typedArray.getColor(R.styleable.VelocimetroView_colorAguja, Color.RED)
            paintDial.color = typedArray.getColor(
                R.styleable.VelocimetroView_colorDial,
                ContextCompat.getColor(context, R.color.md_theme_tertiaryFixedDim)
            )
            paintMarcas.color = typedArray.getColor(
                R.styleable.VelocimetroView_colorMarcas,
                ContextCompat.getColor(context, R.color.md_theme_tertiary)
            )
            paintTextoNumerosDial.color = typedArray.getColor(
                R.styleable.VelocimetroView_colorTextoNumeros,
                ContextCompat.getColor(context, R.color.md_theme_tertiaryFixed)
            )
            paintTextoVelocidadCentral.color = typedArray.getColor(
                R.styleable.VelocimetroView_colorTextoCentral,
                ContextCompat.getColor(context, R.color.md_theme_errorContainer_mediumContrast)
            )
            ...
            longitudAgujaFactor =
                typedArray.getFloat(R.styleable.VelocimetroView_longitudAgujaFactor, 0.85f)
            textSizeNumerosDial =
                typedArray.getDimension(R.styleable.VelocimetroView_textSizeNumerosDial, 30f)
            textSizeVelocidadCentral = typedArray.getDimension(
                R.styleable.VelocimetroView_textSizeVelocidadCentral,
                150f
            ) // Se ajusta en onSizeChanged

            anguloInicioDial =
                typedArray.getFloat(R.styleable.VelocimetroView_anguloInicioDial, 135f)
            anguloBarridoDial =
                typedArray.getFloat(R.styleable.VelocimetroView_anguloBarridoDial, 270f)

            typedArray.recycle()
            paintLed.color = colorLedOff
            startDiscoverabilityAnimation()
        }
        // --- PASO 2: LEER CONFIGURACIÓN DEL USUARIO (SOBREESCRIBE LOS VALORES ANTERIORES) ---
        // Este bloque ahora se ejecuta DESPUÉS de leer el XML.
        val sharedPreferences = PreferenceManager.getDefaultSharedPreferences(context)
        val velocidadMaximaGuardada = sharedPreferences.getString("velocidad_maxima", velocidadMaxima.toString())
        velocidadMaxima = velocidadMaximaGuardada?.toIntOrNull() ?: velocidadMaxima
        ...
        paintTextoUnidad.color =
            paintTextoVelocidadCentral.color // Mismo color que el número por defecto
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        paintDiscoverabilityText.textSize = radio * 0.1f
    }
    fun setDistanceUnit(unit: DistanceUnit) {
        if (currentUnit == unit) return

        currentUnit = unit
        textoUnidadVelocidad = if (unit == DistanceUnit.MI) "mph" else "km/h"

        // Es importante invalidar para que la vista se redibuje con la nueva unidad
        invalidate()
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        for (i in 0..velocidadMaxima step 5) { // Iterar de 5 en 5 para marcas pequeñas
            ...
            val porcentajeDelMaximo =
                if (velocidadMaxima > 0) i.toFloat() / velocidadMaxima.toFloat() else 0f
            ...
            val esMarcaPrincipalNumerada =
                (i % incrementoNumeros == 0 || i == 0 || i == velocidadMaxima)
            val factorLongitudMarca =
                if (esMarcaPrincipalNumerada) 0.88f else 0.92f // Marcas más cortas si no tienen número
            ...
            paintMarcas.strokeWidth =
                if (esMarcaPrincipalNumerada) grosorDial * 0.20f else grosorDial * 0.10f
            ...
            if (esMarcaPrincipalNumerada) {
                ...
                val yTexto =
                    centroY + (radio * factorPosTexto) * sin(anguloRadMarca).toFloat() + (paintTextoNumerosDial.textSize / 3f)
        ...
        val porcentajeVelocidad =
            if (velocidadMaxima > 0) velocidadParaAguja / velocidadMaxima.toFloat() else 0f
        ...
        canvas.rotate(
            ...
            centroX,
            centroY
        ) // Rotar el canvas al ángulo absoluto del dial
...
//        canvas.drawCircle(centroX, centroY, grosorAguja * 0.85f, paintCirculoCentralAguja) // Círculo central más grande
        val radioLed = radio * 0.12f
        canvas.drawCircle(centroX, centroY, radioLed, paintLed)
        ...
        paintTextoVelocidadCentral.getTextBounds(
            textoVelocidadStr,
            0,
            textoVelocidadStr.length,
            rectBoundsVelocidad
        )
        ...
        canvas.drawText(textoUnidadVelocidad, centroX, yPosUnidad, paintTextoUnidad)
        // ### 6. NUEVO: DIBUJAR INDICADOR DE ACCIÓN (PULSO Y TEXTO) ###

        // Guardamos el estado actual del canvas
        canvas.save()
        // Aplicamos la escala y la transparencia de la animación de pulso
        canvas.scale(discoverabilityScale, discoverabilityScale, centroX, centroY)
        paintDiscoverabilityText.alpha = (255 * discoverabilityAlpha).toInt()

        // Dibujamos el texto de ayuda debajo del número de velocidad
//        val textoAyuda = "TOCA PARA INICIAR"
        // Obtener la posición del texto de velocidad
        val ytPosVelocidad = centroY + radio * 0.10f + (rectBoundsVelocidad.height() / 2f)
// Colocar el texto de ayuda justo encima
        val yPosAyuda = ytPosVelocidad - (paintTextoVelocidadCentral.textSize)
        canvas.drawText(textoDeEstado, centroX, yPosAyuda, paintDiscoverabilityText)

        // Restauramos el canvas a su estado original para no afectar a otros dibujos
        canvas.restore()

    }

    // --- MÉTODO DE ESTADO COMPLETAMENTE ACTUALIZADO ---
    fun setRecordingState(isRecording: Boolean) {
        if (this.isRecording == isRecording) return
        this.isRecording = isRecording

        // Siempre cancelamos ambas animaciones para empezar de nuevo
        ledAnimator?.cancel()
        discoverabilityAnimator?.cancel()

        if (isRecording) {
            // --- ESTADO "GRABANDO" ---

            // 1. Texto
            textoDeEstado = "GRABANDO..."

            // 2. Color del texto
            paintDiscoverabilityText.color = colorLedOn // Verde, para que coincida con el LED

            // 3. LED parpadeante (lógica existente)
            paintLed.color = colorLedOn
            ledAnimator =
                ValueAnimator.ofObject(ArgbEvaluator(), colorLedOn, Color.TRANSPARENT).apply {
                    duration = 700
                    repeatMode = ValueAnimator.REVERSE
                    repeatCount = ValueAnimator.INFINITE
                    addUpdateListener {
                        paintLed.color = it.animatedValue as Int
                        invalidate()
                    }
                    start()
                }

            // 4. Iniciar animación de pulso (reutilizada)
            startDiscoverabilityAnimation()
            ...
            // 1. Texto
            textoDeEstado = "TOCA PARA INICIAR"

            // 2. Color del texto (puedes elegir un color neutro o el rojo)
            paintDiscoverabilityText.color = Color.CYAN // O el color que prefieras

            // 3. LED rojo fijo (lógica existente)
            paintLed.color = colorLedOff

            // 4. Iniciar animación de pulso (reutilizada)
            startDiscoverabilityAnimation()

            invalidate() // Asegurarse de redibujar con el nuevo estado
        }
    }

    // --- NUEVA FUNCIÓN PARA LA ANIMACIÓN DE DESCUBRIBILIDAD ---
    private fun startDiscoverabilityAnimation() {
        discoverabilityAnimator = ValueAnimator.ofFloat(0f, 1f).apply {
            duration = 1500 // Animación lenta y sutil
            repeatMode = ValueAnimator.REVERSE
            repeatCount = ValueAnimator.INFINITE
            interpolator = AccelerateDecelerateInterpolator()
            addUpdateListener { animation ->
                val fraction = animation.animatedValue as Float
                // Hacemos que el tamaño vaya de 1.0 a 1.05 (un 5% más grande)
                discoverabilityScale = 1f + fraction * 0.05f
                // Hacemos que la opacidad vaya de 1.0 a 0.7 (un 30% más transparente)
                discoverabilityAlpha = 1f - fraction * 0.3f
                invalidate()
            }
            start()
        }
    ...
    fun setVelocidadConAnimacion(nuevaVelocidad: Float) {
        ...
        animator = ValueAnimator.ofFloat(this.velocidadActual, velocidadLimpia).apply {
            ...
            addUpdateListener { animation ->
                this@VelocimetroLedView.velocidadActual = animation.animatedValue as Float
                invalidate()
            }
            start()
        }
